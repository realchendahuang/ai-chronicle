...
What appeared then was not a general software worker. It was an actionable output channel for a language model. Words could specify a function, and the function could reach beyond the conversation into software already built by others. Tool-calling systems and coding agents would extend that arrangement, with more scaffolding, state, and verification. They would also inherit its unresolved question: when an ambiguous sentence becomes an executable artifact, where does understanding end and responsibility begin?
