...
The OpenAI API changelog recorded the family on 9 July 2026, grouping programmatic tools, explicit caching, persistent reasoning, `max`, Pro mode, and multi-agent beta. Taken together, they moved more orchestration into the native model and API layer. The system was expected to generate, arrange tools, divide work, retain state, and account for the computation used along the way.
