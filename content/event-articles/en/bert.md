...
Then came a deliberate anticlimax. Downstream systems were kept small. Classification read the representation at `[CLS]`. Extractive question answering predicted start and end positions in a passage. Sentence-pair tasks reused the two-segment format. The same pretrained parameters were fine-tuned end to end, with only a thin output layer changing from job to job. BERT Large had 24 layers and about 340 million parameters; across eleven NLP tasks, including GLUE and SQuAD, the paper reported state-of-the-art results.
...
At the center of that shift was a blank. BERT did not become influential by speaking more loudly. It learned by having a word taken away, and by being allowed to look everywhere else before trying to return it.
