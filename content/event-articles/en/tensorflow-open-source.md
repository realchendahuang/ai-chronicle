...
Dataflow graphs also changed deployment: a graph could be frozen for serving or mobile runtimes, shrinking the format gap between research and production. Graph-mode debugging pain was real; dynamic control flow and sparse inputs frustrated newcomers. Community tutorials, wrappers, and TensorBoard turned curves and graphs into a shared debugging language. Google gained a standards foothold and a cloud on-ramp; researchers gained a citable implementation coordinate. Whether TensorFlow later kept interface beauty is another essay. Opening a second-generation internal system under Apache 2.0 rewrote participation thresholds.
