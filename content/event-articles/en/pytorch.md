...
When Facebook AI Research put PyTorch in public view around early 2017, the pitch was not another operator catalog. It was a rhythm for writing models: the computation graph is built as far as the forward pass actually runs.

That style is define-by-run, often called a dynamic graph. You write `for`, `if`, and recursion in Python with tensor ops interleaved; automatic differentiation follows the path that really executed. Against the static-graph experience common at the time—build the whole graph first, then feed data—the dynamic graph sits closer to the pseudocode in a researcher’s head. A debugger can stop on a layer’s output. Shape errors surface on the line that caused them, not only later in a remote session run. For people fluent in NumPy and the scientific Python stack, what drops is the tax of translating an idea into framework dialect.
...
CUDA belongs in the same sentence. Without a stable GPU programming and library stack, a Python framework is a slow interpreter shell. PyTorch rides existing accelerators and teaches habits such as `tensor.to(device)` and keeping module parameters on one device. AlexNet proved the compute path; TensorFlow proved industrial open-source frameworks could work; PyTorch proved research feel could be a framework’s home-field advantage. Stacked together, they form the usual late-2010s deep-learning toolchain collage.
