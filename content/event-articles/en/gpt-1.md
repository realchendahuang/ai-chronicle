...
The paper proposed a two-stage bargain. First, train a twelve-layer Transformer decoder on BooksCorpus to predict the next token. No entailment labels, no answer spans, no sentiment classes—only the pressure to continue written language. Then take that same body into supervised tasks and fine-tune it.
