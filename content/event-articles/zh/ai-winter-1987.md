...
与此同时，通用工作站变得更快、更便宜。Sun 等工作站与不断改进的通用 CPU，使越来越多 AI 开发可以在非专用机器上完成；Common Lisp 与各类编译器也在通用硬件上成熟，削弱了“必须买 Lisp 机器才能做 AI”的销售叙事。Lisp 机器原本以适合人工智能开发的软硬件环境建立优势；当普通设备能够承担越来越多同类工作，专用机器的价格和封闭性便从卖点变成负担。MIT 博物馆收藏的 CADR 等 Lisp 机器，今天看起来像密集布线的文物；在当时，它们是一条即将失去价格优势的产品线。需求下降，供应商收缩，围绕专用硬件、工具与咨询构成的市场在 1987 年后迅速退潮——年份是后设的标记，而不是某日开盘铃响起的崩盘。
...
Lisp 机器市场的收缩有可观察的产业指标：专用硬件厂商裁员、并购或退出，工作站与通用 Unix 环境成为默认开发平台，知识工程工具的许可收入从高峰回落。精确的“崩盘日”并不存在，但 1987 年前后被多家史述选为分界，是因为专用 AI 硬件的经济理由在那时已明显变弱。与此同时，神经网络与统计学习等路线尚未成为企业预算里的替代明星；中间出现的是一段命名谨慎、演示收敛、合同更短的时期。
