---
id: gemini-3-5-flash
title: Gemini 3.5 Flash 发布
subtitle: Flash 档进入持续前沿 Agent 工作
date: '2026-05-19'
datePrecision: day
importance: B
eventType:
  - model
topics:
  - large-language-models
  - ai-agent
  - ai-coding
companies:
  - google
models:
  - gemini-3.5-flash
concepts:
  - llm
  - coding-agent
  - tool-calling
summary: Google 发布 Gemini 3.5 Flash 正式版，把持续高强度 Agent 与代码任务定位为 Flash 主力场景，并将其设为 gemini-flash-latest 的默认模型。
background: Flash 系列最初主要强调速度与成本，但 Agent 和代码任务要求模型在长时间执行中保持推理、工具调用和状态一致性。
whatHappened: Gemini 3.5 Flash 面向持续前沿表现优化，重点覆盖 Agent 与代码工作。Google 同期继续推进托管 Agent 和计算机操作能力，把模型与执行环境更紧密地结合。
whyImportant: 它说明“快速档”不再只承担简单任务。高吞吐模型开始成为长期 Agent 的主力，能力分层逐渐从大小模型转向不同工作负载。
impact:
  developer: 开发者可以用更高吞吐的模型承担持续代码与 Agent 工作。
  industry: 速度、成本和长期任务可靠性的平衡成为模型分层的新标准。
beforeAfter:
  before: Flash 模型主要被视为低延迟、轻任务选项。
  after: Flash 档开始承担持续前沿 Agent 和代码工作。
relatedEvents:
  - gpt-5
  - claude-5-family
  - deepseek-v4
sources:
  - title: Gemini API changelog — May 19, 2026
    url: https://ai.google.dev/gemini-api/docs/changelog
    publisher: Google
    type: official
status: verified
slug: gemini-3-5-flash
---
